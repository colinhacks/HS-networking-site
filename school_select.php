...
unset($_SESSION['upload_error2']);
	...
	$profile_school = get("school_id_1","profile","member_index","".$_SESSION['member_index']);
	echo "row ret"; 
	if($profile_school !== '0'){
	...
	while($row = mysqli_fetch_assoc($schools)){
		...
		echo "
			<td style='border: 0px black solid;width:20%;max-width:200px;height:120px;background-color:white;text-align:center;font-size:small;vertical-align:bottom'>
				<input style='margin:0px;border:0px;' name='".$school_id."' type='checkbox' value='".$school_id."' /> <span style='vertical-align:top'>&nbsp;".$school_name."</span><br/>";
		echo $mascot_pic;
			echo "	<img style='width:95px;height:95px;' src='".$mascot_pic."' />
			</td>
		";
	}
	echo "</tr></table>";
	...
	while($row = mysqli_fetch_assoc($schools)){
		...
		echo "
			<td style='border: 0px black solid;width:20%;max-width:200px;height:120px;background-color:white;text-align:center;font-size:small;vertical-align:bottom;padding-bottom:15px;'>
				<input style='margin:0px;border:0px;' type='checkbox' name='".$school_id."' value='".$school_id."' /> <span style='vertical-align:top'>&nbsp;".$school_name."</span><br/>
				
				<img style='width:95px;height:95px;' src='images/".$mascot_pic."' />
			</td>
		";
	}
	...
	echo "<table style='border:0px black solid;color:#A80000;font-weight:bold;margin:5px;padding:5px;'>";
	...
	echo "<input class='button' type='submit' value='Submit!' action='process_school_select.php' style='text-align:right;border:0px outset #12146B;'/>";
	
	
	echo "</form>";
	echo "<br/><div style='color:#12146B;text-align:center;font-style:bold;'>Don't see your school?<br/>";
	echo "<a class='button' href='generic.php?job=create_school' ><span style='color:white;text-transform:uppercase;padding:1px;border:0px outset #12146B'>create the page</span></a></div>";
	echo "<div style='color:#12146B;text-align:center;font-style:bold;'>(Please only add DoDEA schools that do not currently exist.)<br/><br/>";
